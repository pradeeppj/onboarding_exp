...
// -------------------- CONFIG: AI (Condition B) --------------------
const AI_PREFILL_FUNCTION_URL =
  "https://us-central1-onboarding-dcb39.cloudfunctions.net/prefillBusiness";

// -------------------- CONFIG: CP575 Verified (Condition B only) --------------------
const VERIFIED_CP575 = {
...
const screens = [
  {
    ...
    render: () => `
      ${(condition === "B" || condition === "C") ? aiBannerHtml() : ""}

      <div class="note small">
        <strong>Study instruction:</strong> Use the Business Profile (Reference) to complete the form.
        ${(condition === "B" || condition === "C")
          ? `<div class="muted small" style="margin-top:6px;">In this condition, the system attempts to prefill using CP 575 + AI suggestions.</div>`
          : ""}
      </div>
    ...
    onMount: async () => {
      ...
      if (condition === "B" || condition === "C") {
        // Prefill VERIFIED CP575 fields with 100% confidence (no OCR, no OpenAI)
        values.legalName = VERIFIED_CP575.legalName;
        values.ein = VERIFIED_CP575.ein;
        values.address = VERIFIED_CP575.address;

        ai.fields = ai.fields || {};
        ai.fields.legalName = { value: VERIFIED_CP575.legalName, confidence: 1 };
        ai.fields.ein = { value: VERIFIED_CP575.ein, confidence: 1 };
        ai.fields.address = { value: VERIFIED_CP575.address, confidence: 1 };

        wireAiBannerControls();
        updateAiBannerUI();
        runAiPrefillIfNeeded("auto_on_start").catch(() => {});
      }
    },
    canNext: () => true
  },
...

  {
    ...
    onMount: () => {
      const canAutoFillDesc =
        ...
        (condition !== "C" || (typeof ai.fields?.description?.confidence === "number" && ai.fields.description.confidence >= CONFIDENCE_THRESHOLD_C));
...
async function runAiPrefillIfNeeded(reason) {
  ...
  try {
    const res = await fetch(AI_PREFILL_FUNCTION_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        companyHint: "ConnectIQ Labs",
        dossier: window.__COMPANY_DOSSIER__, // string or object
    ...
    ai.lastHttpStatus = res.status;

    const json = await res.json();

    // ---- UI DEBUG: store raw function response ----
    ai.lastResponseJson = json;
    try {
      ai.lastResponsePretty = JSON.stringify(json, null, 2);
    } catch {
      ai.lastResponsePretty = String(json);
    }
    ...
    if (!res.ok || !json || !json.ok) {
      const msg =
        ...
        `AI prefill failed (HTTP ${res.status})`;
      throw new Error(msg);
    }

    ai.status = "done";
    ai.lastRunTsIso = new Date().toISOString();
    ai.fields = json.fields || {};
    ai.meta = json.meta || null;

    logEvent("ai_prefill_ok", {reason});
    ...
    ai.status = "error";
    ai.error = String(e);
    logEvent("ai_prefill_failed", {message: ai.error});
    updateAiBannerUI();
  }
}
...
function applyAiToFormIfPresent() {
  ...
  Object.keys(values).forEach((key) => {
    ...
    // Condition C: Only fill if confidence >= 60%
    if (condition === "C") {
      if (typeof confidence !== "number") return;
      if (confidence < CONFIDENCE_THRESHOLD_C) return;
    }
    ...
    // If select, pick closest matching option based on CURRENT DOM options
